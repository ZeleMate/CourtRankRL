...
Whenever an AI agent creates a summary of the proposed changes, place that files into the /CourtRankRL/data/Agent summaries folder. Read the necessarry docs from that folder.
...
- **Policy model**: Fine-tune `Qwen/Qwen3-4B-Instruct-2507` using QLoRA adapters (4-bit quantization) for document scoring via autoregressive generation; model receives full chunk context to understand relevance reasoning; use groupwise softmax over candidate scores to produce reranking distribution.
- **Training framework**: Use TRL `GRPOTrainer` for GRPO algorithm configuration; configure for **A100 GPU (80GB VRAM)** with memory-efficient batch settings, gradient accumulation, RSLoRA for stability, and automatic dtype optimization.
- **Reward function**: TRL-compatible signature; extract query_id from prompt via regex, lookup slate metadata in global dict; baseline order is the slate's initial fusion ranking; compute nDCG@10 difference between baseline fusion order and policy order as core reward; add small entropy bonus for exploration; assign negative reward for parse failures (not zero); clip rewards to stable range for gradient stability.
- **Dataset strategy**: Split full dataset into train/eval sets (80/20) with shuffled deterministic seed for reproducibility; each query annotated with 30 documents in qrels for sufficient supervision signal.
- **Outputs**: Save LoRA adapter weights and tokenizer to `data/models/grpo_policy/`; emit reranked document IDs per query; log improvements to `metrics.json`.

GRPO Implementation Plan (TRL-based, A100 memory-efficient, chunk-based)
- **Environment**: Cloud training on A100 GPU (80GB VRAM); local M3 MacBook Air for baseline retrieval and evaluation only. **NOTE**: A100 configuration uses memory-efficient settings.
- **Acceleration framework**: Use Unsloth for model loading (`FastLanguageModel.from_pretrained`) with integrated 4-bit quantization, LoRA adapters, and vLLM inference; Unsloth gradient checkpointing (`use_gradient_checkpointing="unsloth"`) provides 50%+ memory savings enabling stable training on memory-constrained GPUs; vLLM accelerates completions during GRPO rollouts by 2-3x through efficient batched inference and KV-cache optimization; RSLoRA (`use_rslora=True`) ensures training stability; automatic dtype selection (`dtype=None`) optimizes for GPU; total training time on A100 with reduced batches.
...
- **Model configuration**: Load `unsloth/Qwen3-4B-Instruct-2507` with 4-bit quantization; attach QLoRA adapters targeting all attention and MLP projection layers (q,k,v,o,gate,up,down); use - **Prompt strategy**: Structured text format (not JSON) displaying chunk-level candidates with full text; include doc_id, chunk_id, court/domain/year, BM25/FAISS scores; format for clarity while accepting higher token count for meaningful context to prevent truncation of full chunk texts.
...
- **RL Training** (Cloud): Notebook (`grpo_train_runpod.ipynb`) trains GRPO policy from slates → saves adapter weights and metrics on A100 GPU (memory-efficient configuration).
...
Acceptance Criteria
- **Dataset scale**: Use full corpus and complete qrels dataset for final pipeline and evaluation.
...
- **Unsloth integration**: CRITICAL - Use Unsloth `FastLanguageModel` for GRPO training, NOT standard transformers `AutoModelForCausalLM`; Unsloth provides integrated 4-bit quantization, LoRA, and vLLM inference with significant performance improvements
...
- **Cloud execution**: A100 GPU (80GB VRAM, memory-constrained) for embedding generation (`gemma_embedding_runpod.ipynb`) and GRPO training (`grpo_train_runpod.ipynb`) via RunPod notebooks; Unsloth-accelerated GRPO training with vLLM inference (`fast_inference=True`) for 2-3x faster training and 50%+ memory efficiency; GPU acceleration for embedding generation and RL training; FAISS index building and hybrid retrieval now run locally.
...
- **Results**: Document ID lists (baseline and reranked), metrics JSON with comparisons
